...
const buttonVariants = cva(
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium cursor-pointer transition-all duration-200 ease-out focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed active:scale-[0.97] [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "btn-flow-primary text-primary-foreground border-0",
        destructive:
          "bg-destructive text-destructive-foreground shadow-sm hover:bg-destructive/90 hover:shadow-md",
        outline:
          "border border-input bg-background/50 shadow-sm hover:bg-accent/80 hover:text-accent-foreground hover:border-primary/30 backdrop-blur-sm",
        secondary:
      ...
      <Comp
        ...
        onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
          if (!silent && !props.disabled) {
            play(variant === "default" ? "success" : "tap");
          }
          onClick?.(e);
        }}
        {...props}
